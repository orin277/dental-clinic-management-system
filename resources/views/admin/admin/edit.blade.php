        ...
        <div class="relative w-full h-full overflow-y-auto dark:bg-gray-900 lg:ml-64">
            <div class="rounded-lg shadow mx-10 mt-8 px-14 py-8 bg-white border-b border-gray-200">
                <div class="flex flex-col">
                    <x-admin-panel.h2>Редагування адміністратора</x-admin-panel.h2>
                    <form action="{{ route("admin.admins.update", $admin->id) }}" method="post">
                        ...
                        <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                            <div class="w-full">
                                ...
                                <x-form.text-input name="name" id="name" :value="$admin->name" minlength="1" maxlength="30" required />
                                <x-form.input-error :messages="$errors->get('name')" class="mt-2" />
                            </div>
                            <div class="w-full">
                                <x-form.label for="surname">Прізвище</x-form.label>
                                <x-form.text-input name="surname" id="surname" :value="$admin->surname" minlength="1" maxlength="40" required />
                                <x-form.input-error :messages="$errors->get('surname')" class="mt-2" />
                            </div>
                            <div class="w-full">
                                <x-form.label for="patronymic">По батькові</x-form.label>
                                <x-form.text-input name="patronymic" id="patronymic" :value="$admin->patronymic" minlength="1" maxlength="30" required />
                                <x-form.input-error :messages="$errors->get('patronymic')" class="mt-2" />
                            </div>
                            <div class="w-full">
                                <x-form.label for="date_of_birth">Дата народження</x-form.label>
                                <x-form.date-input name="date_of_birth" id="date_of_birth" :value="$admin->date_of_birth" required />
                                <x-form.input-error :messages="$errors->get('date_of_birth')" class="mt-2" />
                            </div>
                            <div class="w-full">
                                <x-form.label for="phone">Номер телефону</x-form.label>
                                <x-form.tel-input name="phone" id="phone" :value="$admin->phone" required />
                                <x-form.input-error :messages="$errors->get('phone')" class="mt-2" />
                            </div>
                            <div class="w-full">
                                <x-form.label for="email">Email</x-form.label>
                                <x-form.email-input name="email" id="email" :value="$admin->email" required />
                                <x-form.input-error :messages="$errors->get('email')" class="mt-2" />
                            </div>
                            <div class="w-full">
                                <x-form.label for="password">Пароль</x-form.label>
                                <x-form.password-input name="password" id="password" :value="$admin->password" required />
