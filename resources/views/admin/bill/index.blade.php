        ...
        <div class="relative w-full h-full overflow-y-auto bg-gray-50 lg:ml-64 dark:bg-gray-900">
            <div class="rounded-t-lg shadow p-6 bg-white block sm:flex items-center justify-between border-b border-gray-200 dark:bg-gray-800 dark:border-gray-700 mx-10 mt-8">
                <div class="w-full mb-1">
                    <div class="mb-4">
                        <x-admin-panel.h1>Рахунки</x-admin-panel.h1>
                    </div>
                    <div class="items-center justify-between block sm:flex md:divide-x md:divide-gray-100 dark:divide-gray-700">
                        ...
                        <x-form.add-link :href='route("$role.bills.create")' id="createButton">
                            Додати
                        </x-form.add-link>
                    </div>
                </div>
                    ...
                    <x-admin-panel.table>
                        ...
                        @foreach($bills as $bill)
                            <x-admin-panel.table.tr>
                                <x-admin-panel.table.td>{{ $bill->appointment_id }}</x-admin-panel.table.td>
                                <x-admin-panel.table.td>{{ $bill->patient_name . ' ' . $bill->patient_surname . ' ' . $bill->patient_patronymic }}</x-admin-panel.table.td>
                                <x-admin-panel.table.td>{{ $bill->dentist_name . ' ' . $bill->dentist_surname . ' ' . $bill->dentist_patronymic }}</x-admin-panel.table.td>
                                <x-admin-panel.table.td>{{ $bill->amount }}</x-admin-panel.table.td>
                                <x-admin-panel.table.td>{{ date('d-m-Y', strtotime($bill->date)) }}</x-admin-panel.table.td>
                                <x-admin-panel.table.edit-td
                                    :href='route("$role.bills.edit", ["bill" => $bill->id])'
                                    :action='route("$role.bills.destroy", $bill->id)'
                                >
                                </x-admin-panel.table.edit-td>
                            </x-admin-panel.table.tr>
                        @endforeach
