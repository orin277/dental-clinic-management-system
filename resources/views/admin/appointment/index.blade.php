                    ...
                    <div
                        ...
                        <div class="flex items-center mb-4 sm:mb-0">
                            <form class="sm:pr-3 flex items-center w-full space-x-3 md:w-auto" action="#" method="GET">
                                <div>
                                    <label for="search-filter" class="sr-only">Пошук</label>
                                    <x-form.text-input
                                        name="filter[search]"
                                        id="search-filter"
                                        class="w-72"
                        ...
                        <x-form.add-link :href='route("$role.appointments.create")' id="createButton">
                            Додати
                        </x-form.add-link>
                    </div>
                </div>
                    ...
                    <x-admin-panel.table>
                        ...
                        @foreach($appointments as $appointment)
                            <x-admin-panel.table.tr>
                                ...
                                <x-admin-panel.table.td>{{ $appointment->dentist_name . ' ' . $appointment->dentist_surname . ' ' . $appointment->dentist_patronymic }}</x-admin-panel.table.td>
                                <x-admin-panel.table.td>{{ $appointment->cabinet }}</x-admin-panel.table.td>
                                ...
                                <x-admin-panel.table.td>{{ $appointment->appointment_status_name }}</x-admin-panel.table.td>
                                ...
                                <x-admin-panel.table.edit-td
                                    :href='route("$role.appointments.edit", ["appointment" => $appointment->id])'
                                    :action='route("$role.appointments.destroy", $appointment->id)'
                                >
                                </x-admin-panel.table.edit-td>
                            </x-admin-panel.table.tr>
                        @endforeach
