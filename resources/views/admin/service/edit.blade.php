        ...
        <div class="relative w-full h-full overflow-y-auto dark:bg-gray-900 lg:ml-64">
            <div class="rounded-lg shadow mx-10 mt-8 px-14 py-8 bg-white border-b border-gray-200">
                <div class="flex flex-col">
                    <x-admin-panel.h2>Редагування послуги</x-admin-panel.h2>
                    <form action="{{ route('admin.services.update', $service->id) }}" method="post">
                        ...
                        <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                            <div>
                                ...
                                <x-form.select id="type_of_service_id" name="type_of_service_id" :options="$typeOfServices" :value="$service->type_of_service_id" />
                                <x-form.input-error :messages="$errors->get('type_of_service_id')" class="mt-2" />
                            </div>
                            <div class="w-full">
                                <x-form.label for="name">Назва послуги</x-form.label>
                                <x-form.text-input name="name" id="name" :value="$service->name" minlength="5" maxlength="50" required />
                                <x-form.input-error :messages="$errors->get('name')" class="mt-2" />
                            </div>
                            <div class="w-full">
                                <x-form.label for="price">Ціна</x-form.label>
                                <x-form.number-input name="price" id="price" :value="$service->price" min="1" max="1000000" required />
