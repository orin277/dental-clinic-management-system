        ...
        <div class="relative w-full h-full overflow-y-auto bg-gray-50 lg:ml-64 dark:bg-gray-900">
            <div class="rounded-t-lg shadow p-6 bg-white block sm:flex items-center justify-between border-b border-gray-200 dark:bg-gray-800 dark:border-gray-700 mx-10 mt-8">
                <div class="w-full mb-1">
                    <div class="mb-4">
                        <x-admin-panel.h1>Стоматологи</x-admin-panel.h1>
                    </div>
                    <div class="items-center justify-between block sm:flex md:divide-x md:divide-gray-100 dark:divide-gray-700">
                        ...
                        <x-form.add-link :href='route("$role.dentists.create")' id="createButton">
                            Додати
                        </x-form.add-link>
                    </div>
                </div>
                ...
                <div class="flex flex-col">
                    <x-admin-panel.table>
                        <x-slot:header>
                            ...
                            <x-admin-panel.table.th>Спеціалізація</x-admin-panel.table.th>
                            <x-admin-panel.table.th>Кабінет</x-admin-panel.table.th>
                        ...
                        @foreach($dentists as $dentist)
                            <x-admin-panel.table.tr>
                                <x-admin-panel.table.td>{{ $dentist->name . ' ' . $dentist->surname . ' ' . $dentist->patronymic }}</x-admin-panel.table.td>
                                <x-admin-panel.table.td>{{ $dentist->dentist_specializations_name }}</x-admin-panel.table.td>
                                <x-admin-panel.table.td>{{ $dentist->cabinet }}</x-admin-panel.table.td>
                                <x-admin-panel.table.td>{{ $dentist->email }}</x-admin-panel.table.td>
                                <x-admin-panel.table.td>{{ $dentist->phone }}</x-admin-panel.table.td>
                                <x-admin-panel.table.td>{{ date('d-m-Y', strtotime($dentist->date_of_birth)) }}</x-admin-panel.table.td>
                                <x-admin-panel.table.td>{{ $dentist->sex === 1 ? 'Ч' : 'Ж' }}</x-admin-panel.table.td>
                                <x-admin-panel.table.edit-td
                                    :href='route("$role.dentists.edit", ["dentist" => $dentist->id])'
                                    :action='route("$role.dentists.destroy", $dentist->id)'
                                >
                                </x-admin-panel.table.edit-td>
                            </x-admin-panel.table.tr>
                        @endforeach
