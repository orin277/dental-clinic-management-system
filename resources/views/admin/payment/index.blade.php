        ...
        <div class="relative w-full h-full overflow-y-auto bg-gray-50 lg:ml-64 dark:bg-gray-900">
            <div class="rounded-t-lg shadow p-6 bg-white block sm:flex items-center justify-between border-b border-gray-200 dark:bg-gray-800 dark:border-gray-700 mx-10 mt-8">
                <div class="w-full mb-1">
                    <div class="mb-4">
                        <h1 class="text-2xl font-inter-bold text-gray-900 sm:text-2xl dark:text-white">Платежі</h1>
                    </div>
                    <div class="items-center justify-between block sm:flex md:divide-x md:divide-gray-100 dark:divide-gray-700">
                        ...
                        <a href="{{ route("{$role}.payments.create") }}" id="createButton" class="font-inter-medium inline-flex items-center text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800" type="button" data-drawer-target="drawer-create-product-default" data-drawer-show="drawer-create-product-default" aria-controls="drawer-create-product-default" data-drawer-placement="right">
                            Додати
                        </a>
                    </div>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                        ...
                        @foreach ($payments as $payment)
                            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                <td class="px-6 py-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $payment->bill_id }}</td>
                                <td class="px-6 py-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $payment->patient_name . ' ' . $payment->patient_surname . ' ' . $payment->patient_patronymic }}</td>
                                <td class="px-6 py-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $payment->amount }}</td>
                                <td class="px-6 py-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ date('d-m-Y', strtotime($payment->date)) }}</td>
                                <td class="px-6 py-4 space-x-2 whitespace-nowrap">
                                    <a href="{{ route("{$role}.payments.edit", ['payment' => $payment->id]) }}" class="font-inter-medium inline-flex items-center text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-full text-sm px-2 py-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z"></path><path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd"></path></svg>
                                    </a>

                                    <form action="{{ route("{$role}.payments.destroy", $payment->id) }}" method="post" class="inline-flex ">
                                        @csrf
                                        @method('delete')
